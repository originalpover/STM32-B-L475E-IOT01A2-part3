...
THINGSBOARD_URL = "https://iot.makowski.edu.pl/api/v1/'!ACCESS_TOKEN!'/telemetry" # uzupełnić access_token
...
def send_to_thingsboard(payload_json, max_retries = 5):
    attempt = 0
    headers = {"Content-Type": "application/json"}
    data = json.dumps(payload_json)
    while attempt < max_retries:
        try:
            info = requests.post(THINGSBOARD_URL, headers=headers, data=data, timeout=5)
            if info.status_code in (200, 201, 204):
                print("No error")
                return True
            else:
                print(f"ThingsBoard error: {info.status_code} {info.text}")
        except requests.RequestException as e:
            attempt+= 1
            print(f"Connection error: {e}")
            time.sleep(1)
    print("Unable to send data to ThingsBoard")
    return False
...
def update(frame):
    ...
    send_to_thingsboard(payload) #przesłanie danych do ThingsBoarda
